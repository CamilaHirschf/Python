...
def create_app():
 ...
 def login():
  ...
  if form.validate_on_submit():
   # Busca al usuario en la lista de usuarios
   user_instance = next((u for u in users if u.username == form.username.data), None)
   if user_instance and check_password_hash(user_instance.password, form.password.data):
